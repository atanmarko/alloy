...
///
/// This corresponds to the [DiffMode] of the [ZeroTracerConfig].
///
...
pub struct ZeroTracerConfig {
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub diff_mode: Option<bool>,
}

impl ZeroTracerConfig {
    /// Returns true if this trace was requested with diffmode.
    #[inline]
...
mod tests {
    ...
    #[test]
    fn test_serialize_zero_trace() {
        let mut opts = GethDebugTracingCallOptions::default();
        opts.tracing_options.config.disable_storage = Some(false);
        opts.tracing_options.tracer =
            Some(GethDebugTracerType::BuiltInTracer(GethDebugBuiltInTracerType::ZeroTracer));
        opts.tracing_options.tracer_config =
            serde_json::to_value(ZeroTracerConfig { diff_mode: Some(true) }).unwrap().into();

        assert_eq!(
            serde_json::to_string(&opts).unwrap(),
            r#"{"disableStorage":false,"tracer":"zeroTracer","tracerConfig":{"diffMode":true}}"#
        );
    }
